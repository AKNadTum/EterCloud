...
@section('content')
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="bg-white p-6 rounded-[var(--radius-lg)] border border-[var(--border)] shadow-sm">
            ...
            <p class="text-3xl font-bold text-gray-900">{{ $stats['active_subscriptions_count'] }}</p>
            <p class="text-sm text-gray-500 mt-1">Abonnements actuellement en cours</p>
        </div>

        <div class="bg-white p-6 rounded-[var(--radius-lg)] border border-[var(--border)] shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-900">Volume (30j)</h3>
            ...
            <p class="text-sm text-gray-500 mt-1">Revenus bruts sur les 30 derniers jours</p>
        </div>
    </div>

    <div class="bg-white rounded-[var(--radius-lg)] border border-[var(--border)] shadow-sm overflow-hidden mb-8">
                ...
                <tbody class="divide-y divide-[var(--border)]">
                    @forelse($stats['latest_invoices'] as $invoice)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $invoice->customer->name ?? $invoice->customer_email }}</div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-gray-500">Aucune facture récente.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="bg-white p-6 rounded-[var(--radius-lg)] border border-[var(--border)] shadow-sm">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Actions Stripe</h3>
