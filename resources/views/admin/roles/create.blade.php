        ...
        <form action="{{ route('admin.roles.store') }}" method="POST">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <x-ui.input type="text" name="name" id="name" value="{{ old('name') }}" required :invalid="$errors->has('name')" placeholder="Ex: Modérateur" />
                        ...
                        @foreach ($permissions as $permission)
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="permission_{{ $permission->id }}" name="permissions[]" type="checkbox" value="{{ $permission->id }}" class="focus:ring-[var(--ring)] h-4 w-4 text-[var(--primary)] border-[var(--border)] rounded" {{ is_array(old('permissions')) && in_array($permission->id, old('permissions')) ? 'checked' : '' }}>
                                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-[var(--border)]">
                    ...
                    <x-ui.button type="submit">Créer le Rôle</x-ui.button>
                </div>
            </div>
        </form>
    </div>
